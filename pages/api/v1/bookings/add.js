...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (
      from_date.toString() === "Invalid Date" ||
      to_date.toString() === "Invalid Date"
    ) {
    ...
    if (req.body.from_date < Date.now())
      return errorResponse(res, "From date must be in the future", 409);
    //from_date must be before to_date
    if (req.body.from_date > req.body.to_date)
      return errorResponse(res, "From date must be before to_date", 409);
    ...
    const booking = await generaldb.query(
      "SELECT * FROM bookings WHERE room_id = ? AND (from_date BETWEEN ? AND ? OR to_date BETWEEN ? AND ?) AND status != ?",
      [
        req.body.roomid,
        req.body.from_date,
        req.body.to_date,
    ...
    if (booking.length > 0)
      return errorResponse(res, "Room is not available", 409);
    let uid = payload.uid;
    if (req.body.userid && payload.role !== "admin")
      return errorResponse(
        res,
    ...
    const bookingdetails = {
      from_date: formatdate(req.body.from_date),
      to_date: formatdate(req.body.to_date),
    ...
    const newbooking = await generaldb.singlerecord(
      "bookings",
      "id",
      latestbooking
    );
